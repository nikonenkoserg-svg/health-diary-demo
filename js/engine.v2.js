...
const Engine = {
  ...
  parseFood(text) {
    ...
    for (const [name, data] of Object.entries(this.FOOD_DB)) {
      let pos = -1;
      // Прямое вхождение — только на границах слова, иначе «сок» матчится в «кусок».
      const directIdx = t.indexOf(name);
      if (directIdx !== -1) {
        const before = directIdx - 1;
        const after = directIdx + name.length;
        const isLetter = c => /[а-яёa-z0-9]/.test(c);
        const leftOk = before < 0 || !isLetter(t[before]);
        const rightOk = after >= t.length || !isLetter(t[after]);
  ...
  parseGlucose(text) {
    if (!text) return null;
    const t = text.toLowerCase();
    // (?<!\d)…(?!\d) — не рвём длинные числа: «150 грамм» не должно дать «15».
    // X.Y / X,Y (всегда) ИЛИ целое X (условия ниже).
    const matches = [...text.matchAll(/(?<!\d)(\d{1,2})(?:[.,](\d{1,2}))?\s*(ммоль|ммл|mmol)?(?!\d)/gi)];
    ...
    for (const m of matches) {
      ...
      if (/^(кг|килограмм|см|сантиметр|лет|года|год|час|часа|часов|ч |раз|блин|штук|ккал|минут|мин |гр|грамм|мл|литр|штук|ложк|стакан|порц|кусоч|дольк|зубчик|анализ|таблет|табл|доз|укол)/.test(after)) continue;
      ...
      // сахарное слово (в пределах ~25 симв.) или после числа единица «ммоль».
      // «Йогурт 150, печенье 70, сахар 6» → возьмёт 6, а не еду.
      const before = t.slice(Math.max(0, m.index - 25), m.index);
      const glucosePrefix = /(сахар|глюкоз|глюкометр|замер|показал|показыва|уровень)/.test(before);
  ...
  parseEventTime(text) {
    ...
    // "только что", "сейчас", "прямо сейчас" — момент отправки = точное время.
    if (/только что|прямо сейчас|сейчас|щас|сию минуту|пару минут назад/.test(t)) {
  ...
  // Применяет уточнения (граммовка, время) к ПОСЛЕДНЕМУ event если в реплике
  // нет продукта но есть данные. Возвращает true если что-то обновили.
  updateLastEventFromContext(text, profile) {
    if (!this._dayEvents || this._dayEvents.length === 0) return false;
    const last = this._dayEvents[this._dayEvents.length - 1];
    if (!last) return false;
    const t = text.toLowerCase();
    let changed = false;

    // Граммовка для default-portion продуктов
    const reAmt = /(\d+(?:[.,]\d+)?)\s*(г|гр|грамм|граммов|мл|миллилитр)(?![а-яё])/i;
    const am = reAmt.exec(t);
    if (am) {
      const grams = parseFloat(am[1].replace(',', '.'));
      const targets = last.foods.filter(f => f.defaultPortion);
      if (targets.length > 0 && grams >= 20 && grams <= 2000) {
        for (const f of targets) {
          const baseKcalPer100 = f.kcal / (f.portion / 100); // восстановить kcal/100г
          f.portion = grams;
          f.kcal = Math.round(baseKcalPer100 * grams / 100);
          f.defaultPortion = false;
        }
        // Пересчитать кривую с новой портацией
        const coeff = this.getCoefficients(profile || {});
        const timeEffect = this.timeOfDayEffect(last.hour);
        coeff.peakModifier *= timeEffect.modifier;
        last.curve = this.mealCurve(last.foods.map(f => ({
          ...f,
          carbs: f.carbs,
          gi: f.gi
        })), coeff);
    ...
    // Уточнение времени
    const parsedTime = this.parseEventTime(text);
    if (parsedTime && !last.timeCertain) {
      last.eventMinute = parsedTime.minute;
      last.timeCertain = parsedTime.certain;
      last.hour = Math.floor(parsedTime.minute / 60);
      last.minute = parsedTime.minute % 60;
      changed = true;
    }
    return changed;
  },
