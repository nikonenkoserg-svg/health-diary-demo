...
Тон: спокойный, внимательный. Никаких "отличный вопрос", "молодец", "рад помочь".`;
  ...
  filterResponse(text, lastUserMsg, isLongAnswerContext) {
    ...
    // «Зафиксировал./Отметил./Принял./Записал.» в начале — это ВТОРАЯ квитанция
    // поверх серой строки-чека. Режем ведущий глагол, ЕСЛИ дальше идёт разбор
    // (оставляем суть). Одиночный акт без текста после не трогаем — там это ответ.
    r = r.replace(/^\s*(зафиксировал|отметил|принял|записал)[,.]\s+(?=\S)/i, '');
